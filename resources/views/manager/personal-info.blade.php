        ...
        <div class="col-12">
            <nav aria-label="breadcrumb" class="d-flex align-items-center">
                ...
                <ol class="breadcrumb my-auto ms-4">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><a href="{{ route('manager.get-personal-info') }}">Thông tin cá nhân</a></li>
                </ol>
            </nav>
        </div>
    </div>
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form method="post" action="{{ route('manager.update-password-api', $user->id) }}" id="update-password-form">
                                        @csrf
...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            function submitFormUpdatePersonalInfo (form, managerId, overlay) {
                const formDataObj = {};
                form.find('input, select, textarea').each(function() {
                    formDataObj[$(this).attr('name')] = $(this).val();
                });

                let url = `{{ route("manager.update-personal-info-api", ":manager") }}`;
                url = url.replace(':manager', managerId);
                $.ajax({
                    type: 'PUT',
            ...
            function submitFormUpdatePassword(form, userId, overlay) {
                ...
                let url = `{{ route("manager.update-password-api", ":userId") }}`;
            ...
            function addEventForButtons() {
                $('#btn-update-personal-info').off('click').click(function(e) {
                    ...
                    submitFormUpdatePersonalInfo(form, managerId, overlay);
                });
